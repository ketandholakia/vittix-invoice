...
class QuoteItemEntrySheet extends ConsumerStatefulWidget {
  final QuoteItemsCompanion? existingItem;
  final bool isInterState;
  final DateTime? hsnLookupDate;

  const QuoteItemEntrySheet({
    super.key,
    this.existingItem,
  ...
  @override
  ConsumerState<QuoteItemEntrySheet> createState() =>
      _QuoteItemEntrySheetState();
}

class _QuoteItemEntrySheetState extends ConsumerState<QuoteItemEntrySheet> {
  ...
  Future<void> _save() async {
    ...
    final item = QuoteItemsCompanion(
  ...
  String? _percentageValidator(String? value, String label) {
    final parsed = double.tryParse(value?.trim() ?? '');
    if (parsed == null) return '$label is required';
  ...
  String? _optionalPercentageValidator(String? value, String label) {
    final trimmed = value?.trim() ?? '';
    if (trimmed.isEmpty) {
      return null;
    }
    return _percentageValidator(trimmed, label);
  }
            ...
            children: [
                ...
                children: [
                  Expanded(
                    ...
                    child: TextFormField(
                      ...
                      validator: (val) =>
                          _requiredPositiveNumber(val, 'Quantity'),
                    ),
                  ),
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      validator: (val) => _requiredPositiveNumber(val, 'Rate'),
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: TextFormField(
                      ...
                      validator: (val) =>
                          _optionalPercentageValidator(val, 'Discount'),
                    ),
                  ),
                ],
              ),
              if (isGstEnabled) ...[
                TextFormField(
                  ...
                  validator: (val) => _percentageValidator(val, 'GST rate'),
                ),
              ],
